...
try:
    ...
    def load_dotenv():
        return False
from publish_current_snapshot import publish_snapshot

load_dotenv()


def fallback_to_latest_snapshot(reason):
    snapshot_date = publish_snapshot()
    print(
        "⚠️ Falling back to the latest archived snapshot "
        f"({snapshot_date}) because the live update did not complete: {reason}"
    )
...
def main():
    ...
    try:
        ...
        if not news_items:
            fallback_to_latest_snapshot("no new articles were returned by Serper")
            return
        ...
        fallback_to_latest_snapshot(str(exc))
